...
export default function PartidoDetailPage() {
  ...
  if (!match) {
    return (
      <div className="max-w-5xl mx-auto px-4 py-16 text-center">
        ...
        <Link href="/partidos" className="text-primary text-sm mt-4 inline-block">← Volver a partidos</Link>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      {/* Back */}
      <Link href="/partidos" className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground transition-colors w-fit">
      ...
      <div>
        <div className="flex items-center gap-3">
          <span className="text-xs font-mono text-primary font-bold bg-primary/10 border border-primary/20 px-2 py-1 rounded-lg">
            #{match.match_number}
          </span>
            ...
            {match.winning_team.length === 0 ? (
              ...
              return (
                <div key={pid} className="flex items-center justify-between gap-2">
                  {p ? (
                    <Link href={`/jugadores/${pid}`} className="text-sm font-medium text-foreground hover:text-primary transition-colors">
                      {p.name}
                    </Link>
                  ) : (
                    <span className="text-sm text-muted-foreground font-mono text-xs">{pid.slice(0, 8)}…</span>
                  )}
                  {p && (
                    <span className="text-xs text-muted-foreground tabular">{p.record}</span>
                  )}
                </div>
              )
            })}
